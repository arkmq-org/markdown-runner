...
const OUTPUT_CHUNK_REGEX = "^```+shell markdown_runner$"
...
// Parse the markdown file name and extract all its chunks.
// The `file` has to exist within the `markdown_dir` folder.
func extractStages(file string) [][]*ExecutableChunk {
	var stages [][]*ExecutableChunk
	var err error
	var fileHandle *os.File
	filepath := path.Join(markdown_dir, file)
...
// Create/update the output chunks for every executed chunk inside the markdown file.
// The markdown file has to exist within the `markdown_dir` folder.
func updateChunkOutput(file string, stages [][]*ExecutableChunk) error {
	// get the path for the input file
	inFPath := path.Join(markdown_dir, file)
	...
	outFPath := path.Join(markdown_dir, file+".out")
...
// Executes every stages, chunks and commands of a file if any
func runMD(file string) error {
	...
	if updateFile && terminatingError == nil {
		terminatingError = updateChunkOutput(file, stages)
		if terminatingError == nil {
			os.Rename(path.Join(markdown_dir, file+".out"), path.Join(markdown_dir, file))
		}
	}
...
func main() {
	...
	flag.StringVar(&markdown_dir, "markdown-dir", "./docs", "where to find the markdown files to execute")
	...
	markdown_files, err := os.ReadDir(markdown_dir)
	pterm.Fatal.PrintOnError(err)
	if noStyling {
	...
	pterm.Fatal.PrintOnError(err)
	env = append(env, "WORKING_DIR="+workding_directory)
