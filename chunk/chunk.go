...
func (chunk *ExecutableChunk) AddCommandToExecute(trimedCommand string, tmpDirs map[string]string) (*RunningCommand, error) {
	...
	ctx, command.CancelFunc = context.WithTimeout(context.Background(), time.Duration(config.MinutesToTimeout)*time.Minute)
	...
	command.Cmd.Env = append(command.Cmd.Env, config.Env...)
